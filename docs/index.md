# workshop-mcp

## References

### Slides

- [やさしい MCP 入門](https://speakerdeck.com/minorun365/yasasiimcpru-men)
- [MCP に入門する/ Introduction to MCP](https://speakerdeck.com/shuntaka/introduction-to-mcp)

### Docs

- [Specification and documentation for the Model Context Protocol](https://github.com/modelcontextprotocol/specification)
- [Part1 : Azure AI Foundry で MCP を使ってみた【深掘りと最新動向調査】](https://zenn.dev/chips0711/articles/e71b088f26f56a)
- [Part2 : Azure AI Foundry で MCP を使ってみた【深掘りと最新動向調査】](https://zenn.dev/chips0711/articles/ba72417a1f6c34)

### Samples

- [AI Gateway Labs with Azure API Management > labs/model-context-protocol](https://github.com/Azure-Samples/AI-Gateway/tree/main/labs/model-context-protocol)

### Tools

- [MCP Inspector](https://github.com/modelcontextprotocol/inspector)

### [Servers](https://github.com/modelcontextprotocol/servers?tab=readme-ov-file#-reference-servers)

- [Playwright MCP server](https://github.com/microsoft/playwright-mcp)
- [AWS MCP Servers](https://github.com/awslabs/mcp)
- [Azure MCP Server](https://github.com/Azure/azure-mcp)
- [GitHub MCP Server](https://github.com/github/github-mcp-server)
